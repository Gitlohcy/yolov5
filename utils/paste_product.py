...
class PasteProduct:
    ...
    def set_mask(self, cls_names:List[str] = None):
        ...
        self.get_mask = (self.get_mask_by_idx if is_same_id
                             else self.get_mask_by_image_id)

    def set_coco(self, coco_path):
        self.coco_path = Path(coco_path)
        assert self.coco_path.is_file() and self.coco_path.suffix == '.json', \
    ...
    def reset_augs(self):
        ...
        self.motion_blur = iaa.MotionBlur(
                        k=self.hyp_dict['m_blur_kernel'],
                        angle=self.hyp_dict['m_blur_angle']
                    )

    def reset_data_dist(self):
        """Set the distribution for number of front img to paste in single back_img"""

        assert len(self.hyp_dict['paste_dist']) == self.hyp_dict['max_paste_n'], \
            'length of paste_dist nid to be same with maximum paste num'

        self.n_products_dist = np.array(random.choices(
                range(1, self.hyp_dict['max_paste_n']+1),
                self.hyp_dict['paste_dist'], k=10000
    ...
    def next_batch(self, n):
        ...
        img_ids = self.get_batch_img_ids(n)
        for id in img_ids:
        ...
        len_ids = len(img_ids)
        [check_len(l, len_ids) for l in [fnames, imgs, masks, cls_ids]]
    ...
    def show_next_batch(self, n, show_mask=True):
        ...
        imgs_to_show = [draw_mask(img, mask) for img, mask, in zip(imgs, masks)] if show_mask else imgs
    ...
    def cache_imgs(self, which):
        if which == 'back':
            ...
            img_is_chosen = self.coco_df['images'][fname_is_exist]['id'].isin(self.anns_df['image_id'])

            img_df = self.coco_df['images']
            self.front_imgs = pd.Series(img_df['file_name'][img_is_chosen].progress_apply(
                                    lambda fname: imageio.imread(self.front_img_path/fname)),
                                index=img_df['id'][img_is_chosen]).to_dict()
    ...
    def paste_front_imgs(self, back_img, num2gen=1):
        ...
        resize_seqs = [[
            ...
            depth_resize,
            ] for depth_resize in depth_level_resize(len(img_segmap_cls), min_size=self.hyp_dict['depth_min_size'])]
        ...
        for resize_seq, (img, segmap, cls_id) in zip(resize_seqs, img_segmap_cls):
            ...
            aug_seqs = append_selective_aug(aug_seqs, cls_id, self.aug_funcs_dict, self.hyp_dict['class_aug'])
        ...
        for i, (img, segmap, cls_id) in enumerate(augmented_small_imgs):
            ...
            aug_during_paste(padded_back_img, bbox, bool_mask, self.motion_blur, active_chance=self.hyp_dict['m_blur_chance'])
        ...
        ## remove overlap bbox
        bbox_to_remove = remove_overlaped_bbox(self.hyp_dict['overlap_thereshold'], augmented_small_imgs, padded_rand_coords, padded_back_img)
